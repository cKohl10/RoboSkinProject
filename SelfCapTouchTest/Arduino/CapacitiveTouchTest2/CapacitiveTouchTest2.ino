...
CapacitiveSensor caps1 = CapacitiveSensor(4, 2);        // 10M resistor between pins 4 & 2, pin 2 is sensor pin, add a wire and or foil if desired
CapacitiveSensor caps2 = CapacitiveSensor(8, 6); 
int readings;
...
void setup()                    
{
  ...
  readings = 0;
   Serial.begin(9600);
   Serial.println("CS_1, CS_2, Time (ms)");
...
void loop()                    
{
  ...
  long time = millis();
  //Serial.print(time);
  String outputData = total1 + delim + total2 + delim + time;
