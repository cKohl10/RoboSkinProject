# Generic plotting for Robotic Skin that sends in bytes of data
# Make sure the number of RX and TX match the arduino! 
# Carson Kohlbrenner
# 7/20/2023
...
rejectionLevel = 0.95 #Percent range of rejection

######### Helper Functions ##########
def readSkinBytes(s):
    value = [0 for i in range(numSensors)]
...
def reset():
    for i in range(20):
        s = ser.read(numSensors)
        print(s)

    for i in range(30):
        s = ser.read(numSensors)
        vals = readSkinBytes(s)
        print(vals)
